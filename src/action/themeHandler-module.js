/**
 * Creates theme-handler behavior with explicit dependencies.
 *
 * @param {Object} options Module dependencies.
 * @param {{ documentElement: { dataset: Record<string, string | null | undefined> } }} options.documentRef Document-like host.
 * @param {{ getItem: (key: string) => string | null; setItem: (key: string, value: string) => void; }} options.localStorageRef Storage implementation.
 * @param {(query: string) => { matches: boolean; addEventListener: (type: "change", listener: (event: { matches: boolean }) => void | Promise<void>) => void; removeEventListener: (type: "change", listener: (event: { matches: boolean }) => void | Promise<void>) => void; }} [options.matchMedia] Match-media factory.
 ...
 * @return {{ handleSwitchColorTheme: () => Promise<unknown> | unknown; initTheme: () => Promise<void> | void; systemColorSchemeListener: (enable?: boolean | null) => Promise<void> | void; }} Theme-handler module API.
 */
export function createThemeHandlerModule({
	...
	/**
	 * Updates DOM/storage state and emits theme message.
	 *
	...
	function messageAndUpdateTheme(theme, updateUserTheme = false) {
		...
		if (updateUserTheme) {
			...
			localStorageRef.setItem("userTheme", theme);
		}
		return sendExtensionMessage({ what: whatTheme, theme });
	}
	 ...
	 * @return {Promise<unknown> | unknown} Theme update result when change is required.
	 */
	...
	function systemColorSchemeListener(enable = true) {
		...
		localStorageRef.setItem("userTheme", "system");
		if (enable) {
			systemColorListener = matchMedia("(prefers-color-scheme: dark)");
