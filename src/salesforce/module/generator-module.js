...
/**
 * Lazily resolves the toast runtime helper to avoid circular module loading.
 *
 * @param {string} message Message key.
 * @param {string} [status=undefined] Optional toast status.
 * @return {Promise<void>}
 */
async function defaultShowToast(message, status = undefined) {
	const toastModule = await import("../toast.js");
	await toastModule.showToast(message, status);
...
/**
 * Returns a valid string value or a safe fallback when the runtime override is not a string.
 *
 * @param {unknown} value Candidate value.
 * @param {string} [fallback=""] Fallback string.
 * @return {string} String value safe for template interpolation.
 */
...
export async function generateSldsToastMessage(
	...
	for (const msg_split of (await getTranslations(message)).split("\n")) {
...
export async function generateManageTabsModal(tabs = [], {
	...
	const divParent = createModalContentContainer(article);
	const wikiLinkTab = `${
		getStringValue(EXTENSION_GITHUB_LINK)
	}/wiki/What-is-a-Tab`;
...
export function createGeneratorModule(overrides = {}) {
	...
	applyOverride(overrides.EXTENSION_GITHUB_LINK, (value) => {
		EXTENSION_GITHUB_LINK = getStringValue(
			value,
