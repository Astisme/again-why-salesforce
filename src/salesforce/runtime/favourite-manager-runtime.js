...
export function createFavouriteManagerModule({
	...
	const extensionLabelRuntime = extensionLabel;
	const extensionNameRuntime = extensionName;
	const hiddenClassRuntime = hiddenClass;
	const salesforceSetupHomeMiniRuntime = salesforceSetupHomeMini;
	const skipLinkDetectionRuntime = skipLinkDetection;
	const tabAddFrontRuntime = tabAddFront;
	const tabAsOrgRuntime = tabAsOrg;
	const toastInfoRuntime = toastInfo;
	const toastWarningRuntime = toastWarning;
	const tutorialEventActionFavouriteRuntime = tutorialEventActionFavourite;
	const tutorialEventActionUnfavouriteRuntime =
		tutorialEventActionUnfavourite;
	...
	async function showFavouriteButton(count = 0) {
		...
		const standardTabs = [
			salesforceSetupHomeMiniRuntime,
			"ObjectManager/home",
		];
		...
		if (
			getWasOnSavedTabRuntime() == null && isCurrentlyOnSavedTab == null
		) {
			await isOnSavedTabRuntime();
		}
		const oldButton = header.querySelector(
			`#${FAVOURITE_BUTTON_ID_RUNTIME}`,
		);
		if (oldButton != null) {
			const allTabs = await ensureAllTabsAvailabilityRuntime();
