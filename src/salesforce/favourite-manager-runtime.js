...
export function createFavouriteManagerModule({
	...
	const FAVOURITE_BUTTON_ID = `${extensionName}-button`;
	const STAR_ID = `${extensionName}-star`;
	const SLASHED_STAR_ID = `${extensionName}-slashed-star`;
	...
	async function generateFavouriteButton() {
		...
		const [connectedCommands, [saveTab, removeTab]] = await Promise.all([
			sendExtensionMessageFn({
				...
				commands: [cmdSaveAsTab, cmdRemoveTab],
			}),
			getTranslationsFn([
				"save_tab",
				"remove_tab",
		...
		const assistiveText = documentRef.createElement("span");
		...
		slashedStar.classList.add(hiddenClass);
		span.appendChild(slashedStar);
		const style = injectStyleFn(
			"awsf-hidden-favman",
			{ css: `.${hiddenClass} { display: none; }` },
		);
	...
	function getFavouriteImage(favouriteId, button = null) {
		...
		return button?.querySelector(`#${favouriteId}`) ??
			documentRef.getElementById(favouriteId);
	}
	...
	function toggleFavouriteButton(isSaved = null, button = null) {
		...
		if (isSaved) {
			...
			slashedStar.classList.remove(hiddenClass);
		} else {
	...
	async function addTab(url) {
		...
		if (
			...
			(
				...
				tabRef.containsSalesforceId(href)
			)
		) {
			org = tabRef.extractOrgName(href);
		}
		await performActionOnTabsFn(
			whatAdd,
			{ label, url, org },
			{
	...
	async function actionFavourite() {
		const url = tabRef.minifyURL(getCurrentHrefFn());
		if (getIsCurrentlyOnSavedTabFn()) {
			const allTabs = await ensureAllTabsAvailabilityFn();
			try {
				const tabToRemove = allTabs.getSingleTabByData({
					url,
					org: tabRef.extractOrgName(getCurrentHrefFn()),
				});
				await performActionOnTabsFn(cxmRemoveTab, tabToRemove);
			} catch (error) {
				consoleRef.warn(error);
				showToastFn("error_remove_not_favourite", toastWarning);
			}
			documentRef.dispatchEvent(
				new customEventCtor(tutorialEventActionUnfavourite),
			);
		} else {
			await addTab(url);
			documentRef.dispatchEvent(
				new customEventCtor(tutorialEventActionFavourite),
			);
		}
	}
	...
	async function showFavouriteButton(count = 0) {
		if (count > 5) {
			const failHead = await getTranslationsFn("error_no_headers");
			consoleRef.error(`${extensionLabel} - ${failHead}`);
			return setTimeoutFn(() => showFavouriteButton(), 5000);
		}
		const currentHref = getCurrentHrefFn();
		const url = tabRef.minifyURL(currentHref);
		const standardTabs = [salesforceSetupHomeMini, "ObjectManager/home"];
		if (standardTabs.includes(url)) {
			return;
		}
		const header = getHeader("div.bRight");
		if (header == null) {
			return setTimeoutFn(() => showFavouriteButton(count + 1), 500);
		}
		const isCurrentlyOnSavedTab = getIsCurrentlyOnSavedTabFn();
		if (getWasOnSavedTabFn() == null && isCurrentlyOnSavedTab == null) {
			await isOnSavedTabFn();
		}
		...
		if (oldButton != null) {
			...
			toggleFavouriteButton(
				allTabs.existsWithOrWithoutOrg({
					url,
					org: tabRef.extractOrgName(currentHref),
				}),
				oldButton,
			);
			return;
