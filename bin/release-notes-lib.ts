...
export async function resolveDefaultFromRef(
	...
	const tagsAtRefArgs = ["tag", "--points-at", toRef];
	const tagsAtRef = await runGit(tagsAtRefArgs);
	if (!tagsAtRef.success) {
		throw new GitCommandError(tagsAtRefArgs, tagsAtRef.stderr);
	}
	const excludedTags = new Set<string>();
	if (tag != null && tag.length > 0) {
		excludedTags.add(tag);
	}
	if (tagsAtRef.stdout.length > 0) {
	...
	const mergedTags = await runGit(mergedTagsArgs);
	if (!mergedTags.success) {
		throw new GitCommandError(mergedTagsArgs, mergedTags.stderr);
	}
	if (mergedTags.stdout.length === 0) {
		return null;
	}
