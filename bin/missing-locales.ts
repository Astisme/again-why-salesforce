 ...
 */
function findMissingKeys(englishFile: MessageFile, localeFile: MessageFile): string[] {
  const missingKeys: string[] = [];
  
  for (const key in englishFile) {
    if (!localeFile.hasOwnProperty(key)) {
...
function checkLocaleFiles(localesDir: string): MissingKeysReport {
  ...
  const localeDirs = fs.readdirSync(localesDir)
    .filter(file => {
      ...
      return fs.statSync(dirPath).isDirectory() && !file.startsWith('en') && file !== "es_419"; // Skip English directories and Spanish 419
    });
  ...
  for (const localeDir of localeDirs) {
    ...
    if (fs.existsSync(localeFilePath)) {
      try {
        ...
        report[`${localeDir}/messages.json`] = ['ERROR_PROCESSING_FILE'];
      }
    } else {
...
function main() {
  ...
  try {
    ...
    const totalMissingKeys = Object.entries(report).reduce((total, [file, missingKeys]) => {
