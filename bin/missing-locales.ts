...
function removeKeysFromLocaleFile(
  ...
  try {
    fs.writeFileSync(filePath, JSON.stringify(updatedLocaleFile, null, "\t"));
  } catch (error) {
  ...
  for (const localeDir of localeDirs) {
    ...
    if (!fs.existsSync(localeFilePath)) {
      ...
      try {
        ...
        report[`${localeDir}/messages.json`] = [error];
      }
  }
...
function main() {
  ...
  try {
    ...
    const totalMissingKeys = Object.entries(report).reduce(
      (total, [file, fileReport]) => {
        if (file === "en/messages.json") return total;
        const localeReport = fileReport as string[];
        return total + localeReport.length;
      },
      0,
    );
