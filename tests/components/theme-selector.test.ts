...
Deno.test("ThemeSelectorAws", async (t) => {
	...
	await t.step("renders both theme buttons and shows the dark toggle in light mode", async () => {
		restoreGlobals();
		...
		const lightButton = component.querySelector('[data-theme-target="light"]');
		const darkButton = component.querySelector('[data-theme-target="dark"]');
	...
	// deno-lint-ignore require-await
	await t.step("dispatches the before-theme-toggle event and swaps visible buttons on click", async () => {
		restoreGlobals();
		globalThis.MutationObserver = class {
			observe() {}
			disconnect() {}
