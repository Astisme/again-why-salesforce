...
Deno.test("readCommitLog throws when git log fails", async () => {
	const runGit: RunGitCommand = (): Promise<GitCommandResult> => {
		return Promise.resolve({
			success: false,
			stdout: "",
	...
	await assertRejects(
		() => readCommitLog("HEAD", "v1.0.0", runGit),
		Error,
		"fatal: bad revision",
	);
});

Deno.test("resolveDefaultFromRef throws when git metadata commands fail", async () => {
	const runGit: RunGitCommand = (
		args: string[],
	): Promise<GitCommandResult> => {
		if (args[0] === "tag") {
			return Promise.resolve({
				success: false,
				stdout: "",
				stderr: "fatal: malformed object name",
			});
		}
		return Promise.resolve({
			success: true,
	...
	await assertRejects(
		() => resolveDefaultFromRef("badref", null, runGit),
		Error,
