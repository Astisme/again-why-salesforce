...
import { 
    ...
    NO_RELEASE_NOTES,
    TAB_ADD_FRONT,
    getStyleSettings,BROWSER,
 GENERIC_TAB_STYLE_KEY, ORG_TAB_STYLE_KEY, LOCALE_KEY,
} from "/constants.js";
...
await Deno.test("bg_getStorage behavior", async (t) => {
	...
	await t.step("get generic Tab style", async () => {
        const genrictabstyle = await bg_getStorage(undefined, GENERIC_TAB_STYLE_KEY);
...
await Deno.test("bg_getSettings behavior", async (t) => {
	await t.step("get all settings", async () => {
        ...
        bg_getSettings(undefined, SETTINGS_KEY, (calledsettings) => assertEquals(calledsettings, settings));
	});
	...
	await t.step("get 2 specific settings", async () => {
        const relatedsettings = await bg_getSettings([NO_RELEASE_NOTES, TAB_ADD_FRONT]);
        assert(relatedsettings != null);
        assert(Array.isArray(relatedsettings));
        assertEquals(relatedsettings.length, 2);
        assert(relatedsettings.filter(s => s.id === NO_RELEASE_NOTES)[0].enabled);
        assertFalse(relatedsettings.filter(s => s.id === TAB_ADD_FRONT)[0].enabled);
        assertEquals(relatedsettings.filter(s => s.id !== NO_RELEASE_NOTES && s.id !== TAB_ADD_FRONT).length, 0);
	});
	...
	await t.step("get specific org tab styles", async () => {
        ...
        const italictabstyle = await bg_getSettings("italic", ORG_TAB_STYLE_KEY);
...
await Deno.test("bg_setStorage behavior", async (t) => {
	...
	await t.step("set with SETTINGS_KEY", async () => {
        await bg_setStorage({ id: "new_setting", enabled: true }, undefined, SETTINGS_KEY);
	...
	await t.step("set with GENERIC_TAB_STYLE_KEY", async () => {
        ...
        await bg_setStorage({ id: "underline", forActive: false, value: "underline" }, undefined, GENERIC_TAB_STYLE_KEY);
        ...
        ], undefined, GENERIC_TAB_STYLE_KEY);
        ...
        await bg_setStorage({ id: "underline", forActive: false, value: "underline" }, undefined, GENERIC_TAB_STYLE_KEY); // reenable for non-active Tab
